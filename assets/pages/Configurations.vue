<template>
    ...
    <main class="container">
        <h2 class="mb-4">{{ $t('hello_configuration') }} <Emoji code="hammer and wrench"></Emoji></h2>

        <div class="card shadow-sm overflow-hidden">
            <header class="card-header d-flex bg-white align-items-center p-lg-4 p-md-3">
                <h5 class="mb-0">{{ $t('configurations') }}</h5>
...
import Edit from "../modules/app/components/Configuration/Edit.vue";
import Delete from "../modules/app/components/Configuration/Delete.vue";
import Show from "../modules/app/components/Configuration/Show.vue";
...
import ConfigurationFactory from "../../src/Factory/Configuration";
import Page from "../modules/common/services/pagination";

import { eventName, countUrl, findUrl } from '../modules/app/components/Configuration/parameters';
...
const fetch = (params = {}) => {
    repository.get(countUrl(), params)
        ...
        .then(data => data.map(ConfigurationFactory.createFromData))
        .then(values => {
            items.value = values;
...
const editResource = item => {
    emitter.emit(eventName('edit'), { ...item, instance: item.instance._id });
};
</script>
